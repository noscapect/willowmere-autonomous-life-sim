...
The asynchronous `CognitionCoordinator` is outside `tickWorld`: it uses a fair round-robin queue, permits the configured number of in-flight local requests, applies a real-time per-agent cooldown, and builds each request from the newest world snapshot when it leaves the queue. A reset, provider change, or model change aborts and invalidates active work with an epoch token, so late responses cannot be applied to a new generation. `applyExternalDecision` clones and revalidates a proposal before it becomes a plan. Model text can never mutate simulation truth. An emergency reflex provider maintains survival when remote/local inference is pending or unavailable.

Mind context contains needs, current perception, compact retained knowledge, bounded memories, and legal candidates. Known people are not candidate targets unless they are currently perceived; context never includes their coordinates. Optional model utterances are treated as short dialogue intent and are emitted only after a physical, validated `TalkTo` succeeds.
