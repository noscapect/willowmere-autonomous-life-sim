...
Current milestone: Mind Runtime Hardening. `server.ts` is the local browser-to-provider boundary. `src/cognition/contracts.ts` defines candidate-bound Zod decisions and perception-only context; `src/runtime/cognitionCoordinator.ts` fairly queues configured concurrent requests, applies a real-time per-agent cooldown, rebuilds context from the latest world snapshot, and aborts/discards work across resets or provider/model switches. `src/runtime/persistence.ts` stores versioned serializable world snapshots. Critical thirst/hunger/energy use `EmergencyReflexProvider` even when LLM cognition is unavailable. The Mind UI saves only non-secret, per-provider model preferences locally.
